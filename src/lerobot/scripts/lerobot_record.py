...
def record_loop(
    ...
    print(f"[record_loop ENTER] control_time_s={control_time_s} task={single_task!r} "
      f"flags(exit_early={events.get('exit_early')}, rerecord={events.get('rerecord_episode')}, stop={events.get('stop_recording')})",
      flush=True)
    ...
    while timestamp < control_time_s:
        ...
        if events["exit_early"]:
            ...
            print(f"[record_loop EXIT_EARLY] elapsed={elapsed:.3f}s -> clearing exit_early and breaking", flush=True)
    ...
    elapsed = time.perf_counter() - start_episode_t
    print(f"[record_loop RETURN] elapsed={elapsed:.3f}s "
      f"flags(exit_early={events.get('exit_early')}, rerecord={events.get('rerecord_episode')}, stop={events.get('stop_recording')})",
      flush=True)
